...
class Controller:
	"""Implementation of Kinematic- and Trajectory-based reference controller

	Refer to the research paper to understand more.
	"""
	...
	def get_starting_joint_angles(self):
		return self.joint_angles(t=0)

	def joint_angles(self, t):
		k = int(((t % self.period) / self.period) * self.array_dim)
		# print('t=',t,'\tk=',k) # DEBUG print the time step
		joint_angles = self.angles[:, k]
		return joint_angles
...
def reshape(x):
	...
	To be used with MAP-Elites code that only outputs values between 0 and 1

	Args:
		x: paramters for the controller
	
	Returns:
		height: height parameter for controller 
		velocity: velocity parameter for controller 
		leg_params: other parameters for controller
	"""
	x = np.array(x)
